...
class Image_GD_Driver extends Image_Driver {
    ...
    public function watermark($properties_arr) {
	    $img = $this->tmp_image;
    ...
    public function watermarkText($properties_arr) {
	    $img = $this->tmp_image;

        foreach($properties_arr as $properties)
        {
		    $attr = $properties['textAttr'];
            ...
            { 
               ...
               $attr['fontColor'] = ImageColorAllocate($img,  $R , $G , $B ); 
            } 
            /*
            $attr = array(
                "fontName"=>SAE_MicroHei, "fontSize"=>12,  "fontColor"=>"333333"
            );
            */

	        imagettftext ($img,$attr['fontSize'],'0', $properties['left'], $properties['top'] ,$attr['fontColor'],$attr['fontName'],$properties['text']);
